...
function isManagedDdnsRemark(value, name) {
  return hasRemarkPrefix(value, `${GD_DDNS_RULE_PREFIX}:${name}`);
}
...
function isLegacyManagedCliRemark(value, remark) {
  return hasRemarkPrefix(value, remark) && parseRuleTimestamp(value) !== null;
}
...
function findRuleByProtocolPortSource({ rules, protocol, port = PORT_RANGE, sourceCidrIp, protocolField, portField, cidrField = 'sourceCidrIp' }) {
  const sourceCidrIpNorm = normalizeIpForCompare(sourceCidrIp);
  return rules.find(rule => (
    normalizeProtocol(getRuleField(rule, protocolField)) === protocol &&
    getRuleField(rule, portField) === port &&
    normalizeIpForCompare(getRuleField(rule, cidrField)) === sourceCidrIpNorm
  ));
}
