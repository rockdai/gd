...
async function main() {
  ...
  // 只有真的对账到机器的那一轮才算"打过全貌"——启动时取 IP 失败/列举抛错的轮次什么都没打，不能把 verbose 消耗掉。
  let verbose = true;
  for (;;) {
    try {
      const summary = await runOnce({ config, logger: console, verbose });
      if (summary.targets > 0) verbose = false;
    } catch (err) {
