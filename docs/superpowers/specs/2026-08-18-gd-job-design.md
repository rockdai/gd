...
5. 对每台 target（串行）：
      ...
      b. 清：rules 中 协议∈{TCP,UDP} 且 端口=1/65535 且 isManagedJobRemark(remark, label)
           且 源IP ≠ 当前IP 的，批量删除
      c. 判定归属：a 中"已存在"的协议，若 rules 里覆盖当前 IP 的不是自己的
           gd-job:<label> 规则 → 该机器记为 pending（不算失败，info 日志）
6. 所有 target 均成功且无 pending → lastIp = ip
   有任一失败或 pending      → 不更新 lastIp，下一轮重试
```
...
`lastIp` 只在一轮**收敛**后才更新。收敛的定义有两个条件：

1. 所有目标机器都没有失败——否则一次瞬时失败会让程序误以为已同步完成，在 IP 不变的情况下永远不再重试。
2. 每台机器的每个协议上，覆盖当前 IP 的规则都是自己的 `gd-job:<label>` 规则。

第二条针对一个真实场景：用户此前在家用 Web PWA 点过「添加白名单」，家里 IP 上有一条 `gd-web` 规则。gd-job 启动后预检发现规则已存在（预检不区分是谁的，阿里云也不允许同一五元组重复建规则），于是不创建自己的。若此时就记下 `lastIp`，24 小时后 Web 的 TTL 清理删掉那条 `gd-web` 规则，家里 IP 上就没有任何规则了，而 gd-job 因为「IP 没变」永远不再检查——锁死到 IP 变化或容器重启为止。迁移到 gd-job 的头一天恰恰最容易撞上。

因此「已存在」的规则若不是自己的，这台机器只算 pending：不报错、打一行 info、不记 `lastIp`，下一轮再来。等那条外来规则消失（`gd-web` 到期被清），gd-job 在下一轮就补上自己的规则，最多一个间隔的窗口。

代价：若覆盖家里 IP 的是一条**永久**的手工规则，gd-job 会每轮重检、每轮打一行相同的 info。日志里会明确提示两条出路：删掉那条手工规则让 gd-job 接管，或用 `MACHINE_DENY` 把这台机器排除。

### 9.1 已知盲区

`lastIp` 一旦记下，在 IP 变化前程序不会再列举机器和规则。因此以下变化不会被自动感知：

- 新创建的 ECS / 轻量服务器
- 有人在控制台手动删除了 gd-job 的规则
- 一台机器从 `MACHINE_DENY` 里移除（需要重启容器让新配置生效，重启本身就会触发全量同步）

处理方式：`docker restart gd-job`。`lastIp` 只在内存里，重启后第一轮必然是全量同步。README 中写明。不为此引入定期强制全量重检——那会与「IP 未变则不重复执行」的要求相抵触，且这些变化都是用户自己主动做的操作，重启一下容器是可接受的代价。
...
- 覆盖当前 IP 的是自己的规则 → 记录 `lastIp`；是别人的规则 → 不记录、pending、下一轮补上后收敛
- `ownsCurrentIpRule` 对 label / IP / ECS 与 SWAS 字段名的判定
